...
export function mapTopPlayerToStats(item: TopPlayerItem): PlayerStats {
  ...
  return {
    ...
    avatar: getAvatarForPlayer(item.playerId),
    id: item.playerId,
    losses,
    rank: item.rank,
    ...
    winRate:
      item.gamesPlayed === 0 ? 0 : (item.gamesWon / item.gamesPlayed) * 100,
    wins: item.gamesWon,
  };
}

export function Leaderboard() {
  const [selectedPlayer, setSelectedPlayer] = useState<TopPlayerItem | null>(
    null
  );
  const {
    error,
  ...
  const players = useMemo(
    () => rawPlayers.map(mapTopPlayerToStats),
    [rawPlayers]
  );
    ...
    <div className="w-full">
      <div className="mb-8">
        ...
        <p className="text-muted-foreground">
          All-time results from completed Panda Monopoly games
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        <LeaderboardStats
          players={players}
          totalRankedPlayers={pagination?.total ?? 0}
        />
      </div>
      ...
      <LeaderboardTable
        ...
        onPlayerClick={(player) => {
          setSelectedPlayer(
            rawPlayers.find((candidate) => candidate.playerId === player.id) ??
              null
          );
        }}
      />
