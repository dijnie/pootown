...
export function AccountMenuButton() {
  ...
  if (!authenticated) {
    return (
      <Button size="lg" disabled={!ready} onClick={openLogin}>
        Sign in
      </Button>
    );
  }
    ...
    <div className="flex items-center gap-3">
      <Button variant="neutral" disabled className="max-w-64 min-w-0">
        <span className="truncate">
          {loading
            ? "Loading account coin..."
            : error
            ? "Account coin unavailable"
            : `${balance?.availableCoin ?? "0"} Account Coin`}
        </span>
      </Button>
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <Button aria-label="Open account menu">
            <UserAvatar playerId={userId} size="xs" />
        ...
        >
          ...
          <div className="border-b px-2 py-2 text-xs">
            ...
            <p>
              Reserved in sessions:{" "}
              <span className="break-all font-semibold">
                {balance?.reservedCoin ?? "0"} Account Coin
              </span>
            </p>
            <p className="mt-1 text-muted-foreground">
              Account Coin is in-app and cannot be withdrawn.
            </p>
          </div>
